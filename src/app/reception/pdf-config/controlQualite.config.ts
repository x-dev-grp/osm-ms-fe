...
export function getControlQualitePdfConfig(delivery: UnifiedDelivery, deliveryType: string): PdfConfig {
  ...
  const title = isOil ? 'PDF.FICH_CONTROL_QUALITE_HUIL' : isOlive ? 'PDF.FICH_CONTROL_QUALITE_OLIVE' : 'PDF.FICH_CONTROL_QUALITE';
  ...
  const pickLabel = (ruleKey: string, frFallback: string): string => {
    ...
    const fromTranslation = (r?.rule as any)?.labelTranslation ?? (r as any)?.labelTranslation;
    const fromLabelAttr = (r?.rule as any)?.label ?? (r as any)?.label ?? r?.rule?.ruleName;
  ...
  const getValue = (result?: QualityControlResultDto): string => (result?.measuredValue != null ? `${result.measuredValue}` : 'N/A');
