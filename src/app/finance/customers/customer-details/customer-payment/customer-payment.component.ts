...
import {
  Currency, FinancialTransaction,
  PaymentMethod,
  TransactionDirection,
  TransactionType
} from '../../../../finance/models/financial-transaction.model';
...
export class CustomerPaymentComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.paymentForm
      ...
      debounceTime(200),
      tap((amt: number) => {
        this.remainingAmount = this.totalAmount - Number(amt || 0);
      })
    )
      .subscribe();
  }
  ...
  ngOnInit(): void {
    ...
    this.paymentForm.patchValue({ amount: Number(this?.totalAmount) });
  ...
  loadBankAccounts(): void {
    const searchData: SearchData = { page: 0, searchData: { operation: SearchOperation.AND, search: { isDeleted: { equalValue: false } } } };
    this._searchService
  ...
  processPayment() {
    ...
    switch (rawMethod) {
      case 'check':         mappedMethod = PaymentMethod.CHEQUE;   break;
      case 'bank_transfer': mappedMethod = PaymentMethod.TRANSFER; break;
      case 'cash':
      default:              mappedMethod = PaymentMethod.CASH;
    }
