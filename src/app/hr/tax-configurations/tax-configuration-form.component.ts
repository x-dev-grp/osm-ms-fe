...
@Component({
  selector: 'app-tax-configurations-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './tax-configuration-form.component.html',
  styleUrl: './tax-configuration-form.component.scss'
})
export class TaxConfigurationFormComponent implements OnInit {
  private readonly fb = inject(FormBuilder);
  private readonly route = inject(ActivatedRoute);
  private readonly router = inject(Router);
  private readonly entityService = inject(TaxConfigurationService);
  ...
  save(): void {
    ...
    const payload: TaxConfiguration = {
      ...raw,
    ...
    request$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (response) => {
        ...
        if (response?.success) {
          this.toast.success();
          this.router.navigate(['/hr/tax-configurations']);
          return;
        }
