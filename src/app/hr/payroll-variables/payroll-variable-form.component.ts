...
import { PayrollVariableService } from '../services/payroll-variable.service';
...
@Component({
  selector: 'app-payroll-variables-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './payroll-variable-form.component.html',
  styleUrl: './payroll-variable-form.component.scss'
})
export class PayrollVariableFormComponent implements OnInit {
  private readonly fb = inject(FormBuilder);
  private readonly route = inject(ActivatedRoute);
  private readonly router = inject(Router);
  private readonly entityService = inject(PayrollVariableService);
  ...
  employees: Employee[] = [];
  statusOptions = ["DRAFT","APPROVED","REJECTED","CONSUMED_IN_PAYROLL"];
  ...
  save(): void {
    ...
    request$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (response) => {
        ...
        if (response?.success) {
          this.toast.success();
          this.router.navigate(['/hr/payroll-variables']);
          return;
        }
