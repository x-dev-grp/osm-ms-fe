...
export class EmplacementDetailComponent implements OnInit {
  ...
  toActif(): void {
    ...
    const action = this.emplacement.actif ? 'désactiver' : 'activer';
    const message = `Voulez-vous vraiment ${action} cet emplacement ?`;

    if (confirm(message)) {
      this.actionEnCours = true;

      const request = this.emplacement.actif
        ? this.emplacementService.desactiverEmplacement(this.emplacement.id)
        : this.emplacementService.activerEmplacement(this.emplacement.id);

      request.subscribe({
        next: (updated) => {
          this.emplacement = updated;
          this.actionEnCours = false;
          console.log(`Emplacement ${action} avec succès`);
        },
        error: (err) => {
