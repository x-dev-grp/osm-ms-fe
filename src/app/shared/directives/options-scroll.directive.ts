...
export class OptionsScrollDirective {
  ...
  constructor(private readonly autoComplete: MatAutocomplete) {
    this.autoComplete.opened.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => {
    ...
    this.autoComplete.closed.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(() => this.removeScrollEventListener());
