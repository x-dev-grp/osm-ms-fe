      ...
      return {
        ...
        export(exportType: string) {
          ...
          const fieldsToExport: {
            ...
            .checkedExportFields()
            .filter((field) => !field.field.flattedList)
            .map((field: any) => {
              return {
                name: field.field.fieldType == FieldType.autocomplete ? field.field.name + '.' + field.field.valuePath : field.field.name,
                label:
                  field.field?.exportLabel ??
                  (field.field?.labelTranslatePath ? translate.instant(field.field?.labelTranslatePath) : field.field.label),
                enumValue: field.field.attributeType == 'enum' ? true : false,
                enumValues:
                  field.field.options?.reduce((acc: any, option: any) => {
          ...
          const flattedAttributes: any[] = store
            .checkedExportFields()
            .filter((field) => field.field.flattedList == true)
            .map((field: { field: Field; checked: boolean }) => {
              return {
          ...
          return _http
            .post(`${environment.apiUrl}/api/${store.endpoint()}/export/${exportType}`, exportDetails, {
              responseType: 'blob',
              observe: 'response'
            })
            .pipe(
        ...
        fetchData() {
          ...
          _searchService
            .search(store.searchData(), store.endpoint())
            .pipe(
              tap((response: SearchResponse) => {
                console.log('Data fetched:', response);
                const totalsObj = response.totals ?? {};
                const totalsMap = new Map<string, number>(Object.entries(totalsObj).map(([k, v]) => [k, Number(v) || 0]));
        ...
        setData: (data: SearchResponse) => {
          ...
          const baseCols = store.allFields().filter((f: Field) => f.dataTable && !f.flattedList);
          ...
          const flatChildren: Field[] = flatParents.flatMap((parent: Field) => {
            const rowWithValues = data.data.find((item: any) => item[parent.name] != null && item[parent.name]?.length > 0);
          ...
          const sorted = [...flatChildren].sort((a, b) => collator.compare(a?.name ?? '', b?.name ?? ''));
