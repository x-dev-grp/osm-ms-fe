...
import { UnitType, OilType } from '../pipes/unit-converter.pipe';
...
export class UnitConverterService {
   ...
   */
  convert(value: number | null | undefined, fromUnit: UnitType, toUnit: UnitType, oilType?: string, precision: number = 2): number {
    ...
    if (fromUnit === 'KG' && toUnit === 'L') {
      ...
      // Invalid conversion
      console.warn(`[UnitConverterService] Invalid conversion from ${fromUnit} to ${toUnit}`);
      return value;
    }
   ...
   */
  convertWithUnit(value: number | null | undefined, fromUnit: UnitType, toUnit: UnitType, oilType?: string, precision: number = 2): string {
    const convertedValue = this.convert(value, fromUnit, toUnit, oilType, precision);
   ...
   */
  convertWithUnitAndOilType(value: number | null | undefined, fromUnit: UnitType, toUnit: UnitType, oilType: string, precision: number = 2): string {
    const convertedValue = this.convert(value, fromUnit, toUnit, oilType, precision);
